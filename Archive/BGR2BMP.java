...
{
   /*
    * Methods to go between little and big endian integer formats since BMP files
    * are little-endian Intel standard (Low-High) while Java is big-endian Motorola
    * (High-Low)
    */
   ...
   public static void main(String[] args) {
      ...
      if (args.length != 5)
         System.out.printf("Usage: java BGR2BMP gray/color width/height length input_byte_file output_BMP_file\n\n");
      else {
         ...
         if (colorImage)
            System.out.printf("BGR color ");
         else
            System.out.printf("gray scale ");

         System.out.printf("file '%s' (width/height = %d, length = %d) and writing out '%s'.\n", inFileName, width, height,
               outFileName);
          ...
          * include the color table or the masks mentioned in the biClrUsed member.
          ...
          ** biCompression - Specifies the type of compression for a compressed bottom-up
          * bitmap (top-down DIBs cannot be compressed). It is 0 in our case.
          ** biSizeImage - Specifies the size, in bytes, of the image. This value will be
          * the number of bytes in each scan line which must be padded to
          ** ensure the line is a multiple of 4 bytes (it must align on a DWORD boundary)
          * times the number of rows.
          ** This value may be set to zero for BI_RGB bitmaps (so you cannot be sure it
          * will be set).
          ...
          ** biClrImportant - Specifies the number of color indexes required for
         ...
         {
            FileInputStream fInStream = new FileInputStream(inFileName); // Open the file that is the second command
                                                                         // line parameter
            ...
            try {
               ...
               /*
                ** Read in 1 or 3 byte values and then write them our as an RGB_Quad value for a
                * 32-bit BMP image which are in the order Blue-Green-Red-Zero.
                */
               EOF = false;
               while (!EOF) {
